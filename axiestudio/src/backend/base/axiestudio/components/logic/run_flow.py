...
class RunFlowComponent(RunFlowBaseComponent):
    display_name = "Kör flöde"
    description = (
        "Skapar en verktygskomponent från ett flöde som tar alla dess inmatningar och kör det. "
        " \n **Välj ett flöde för att använda verktygsläget**"
    )
    ...
    async def update_build_config(self, build_config: dotdict, field_value: Any, field_name: str | None = None):
        if field_name == "flow_name_selected":
            ...
            if missing_keys:
                msg = f"Saknade obligatoriska nycklar i build_config: {missing_keys}"
            ...
            if field_value is not None:
                try:
                    ...
                    msg = f"Fel vid byggande av graf för flöde {field_value}"
