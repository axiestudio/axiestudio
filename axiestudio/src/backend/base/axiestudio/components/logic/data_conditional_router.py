...
class DataConditionalRouterComponent(Component):
    display_name = "Villkor"
    description = "Dirigera dataobjekt baserat på ett villkor som tillämpas på en specificerad nyckel, inklusive boolesk validering."
    icon = "split"
    ...
    inputs = [
        DataInput(
            name="data_input",
            display_name="Datainmatning",
            info="Dataobjektet eller listan med dataobjekt att bearbeta",
            is_list=True,
        ),
        MessageTextInput(
            name="key_name",
            display_name="Nyckelnamn",
            info="Namnet på nyckeln i dataobjektet/dataobjekten att kontrollera",
        ),
        DropdownInput(
            name="operator",
            display_name="Operator",
            options=["equals", "not equals", "contains", "starts with", "ends with", "boolean validator"],
            info="Operatorn att använda för att jämföra värdena. 'boolean validator' behandlar värdet som en boolean.",
            value="equals",
        ),
        MessageTextInput(
            name="compare_value",
            display_name="Matchningstext",
            info="Värdet att jämföra mot (används inte för boolean validator)",
        ),
    ]

    outputs = [
        Output(display_name="Sant utdata", name="true_output", method="process_data"),
        Output(display_name="Falskt utdata", name="false_output", method="process_data"),
    ]
