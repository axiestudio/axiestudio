...
class LangChainHubPromptComponent(Component):
    ...
    description: str = "Prompt-komponent som använder LangChain Hub-prompter"
    beta = True
    icon = "LangChain"
    trace_type = "prompt"
    ...
    inputs = [
        SecretStrInput(
            ...
            display_name="Din LangChain API-nyckel",
            info="LangChain API-nyckeln att använda.",
            required=True,
        ),
        StrInput(
            name="langchain_hub_prompt",
            display_name="LangChain Hub-prompt",
            info="LangChain Hub-prompten att använda, t.ex. 'efriis/my-first-prompt'",
            refresh_button=True,
            required=True,
        ),
    ]
    ...
    outputs = [
        Output(display_name="Bygg prompt", name="prompt", method="build_prompt"),
    ]
    ...
    def update_build_config(self, build_config: dict, field_value: str, field_name: str | None = None):
        ...
        for custom_field in custom_fields:
            new_parameter = DefaultPromptField(
                ...
                info="Fyll i värdet för {" + custom_field + "}",
            ).to_dict()
    ...
    def _fetch_langchain_hub_template(self):
        ...
        if not self.langchain_api_key:
            msg = "Vänligen ange en LangChain API-nyckel"
