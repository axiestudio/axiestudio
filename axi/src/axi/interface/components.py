...
from axi.custom.utils import abuild_custom_components, create_component_template
from axi.log.logger import logger

if TYPE_CHECKING:
    from axi.services.settings.service import SettingsService
...
async def import_AXIESTUDIO_components():
    """Asynchronously discovers and loads all built-in AxieStudio components with module-level parallelization.

    Scans the `AXI.components` package and its submodules in parallel, instantiates classes that are subclasses
    ...
    modules_dict = {}
    try:
        import axi.components as components_pkg
    except ImportError as e:
        await logger.aerror(f"Failed to import AxieStudio.components package: {e}", exc_info=True)
...
async def get_and_cache_all_types_dict(
    ...
    Supports both full and partial (lazy) loading. If the cache is empty, loads built-in AxieStudio
...
async def load_single_component(component_type: str, component_name: str, components_paths: list[str]):
    ...
    from axi.custom.utils import get_single_component_dict
...
def get_all_components(components_paths, *, as_dict=False):
    """Get all components names combining native and custom components."""
    # Import here to avoid circular imports
    from axi.custom.utils import build_custom_components
