...
from axi.field_typing.constants import CUSTOM_COMPONENT_SUPPORTED_TYPES, DEFAULT_IMPORT_STRING
from axi.log.logger import logger

_AXIESTUDIO_IS_INSTALLED = False

with contextlib.suppress(ImportError):
    import AxieStudio  # noqa: F401

    _AXIESTUDIO_IS_INSTALLED = True
...
def _create_AXIESTUDIO_execution_context():
    ...
    try:
        from axi.custom import Component
    ...
    try:
        from axi.io import HandleInput, Output, TabInput
...
def prepare_global_scope(module):
    ...
    for node in import_froms:
        ...
        # If original module starts with AxieStudio, also try AXI equivalent
        if node.module and node.module.startswith("AxieStudio."):
            AXI_module_name = node.module.replace("AxieStudio.", "AXI.", 1)
            module_names_to_try.append(AXI_module_name)
...
def get_default_imports(code_string):
    ...
    AXIESTUDIO_module = importlib.import_module("AXI.field_typing")
