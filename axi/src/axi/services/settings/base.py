...
from axi.constants import BASE_COMPONENTS_PATH
from axi.log.logger import logger
from axi.serialization.constants import MAX_ITEMS_LENGTH, MAX_TEXT_LENGTH
from axi.services.settings.constants import VARIABLES_TO_GET_FROM_ENVIRONMENT
from axi.utils.util_strings import is_valid_database_url
...
class Settings(BaseSettings):
    ...
    knowledge_bases_dir: str | None = "~/.AxieStudio/knowledge_bases"
    """The directory to store knowledge bases."""

    dev: bool = False
    """If True, AxieStudio will run in development mode."""
    database_url: str | None = None
    """Database URL for AxieStudio. If not provided, AxieStudio will use a SQLite database.
    ...
    """If True, AxieStudio will retry to connect to the database if it fails."""
    pool_size: int = 20
    ...
    """If set to True, AxieStudio will expose Prometheus metrics."""
    prometheus_port: int = 9090
    """The port on which AxieStudio will expose Prometheus metrics. 9090 is the default port."""
    ...
    with the same name in case AxieStudio fails to retrieve the variable value."""
    ...
    """If set to True, AxieStudio will not serve the frontend."""

    # Telemetry
    do_not_track: bool = False
    """If set to True, AxieStudio will not track telemetry."""
    telemetry_base_url: str = "https://AxieStudio.gateway.scarf.sh"
    transactions_storage_enabled: bool = True
    """If set to True, AxieStudio will track transactions between flows."""
    vertex_builds_storage_enabled: bool = True
    """If set to True, AxieStudio will keep track of each vertex builds (outputs) in the UI for any flow."""

    # Config
    host: str = "localhost"
    """The host on which AxieStudio will run."""
    port: int = 7860
    """The port on which AxieStudio will run."""
    workers: int = 1
    """The number of workers to run."""
    log_level: str = "critical"
    """The log level for AxieStudio."""
    log_file: str | None = "logs/AxieStudio.log"
    """The path to log file for AxieStudio."""
    ...
    """If set to True, AxieStudio will open the browser on startup."""
    auto_saving: bool = True
    """If set to True, AxieStudio will auto save flows."""
    auto_saving_interval: int = 1000
    """The interval in ms at which AxieStudio will auto save flows."""
    ...
    """If set to False, AxieStudio will not enable the MCP server."""
    mcp_server_enable_progress_notifications: bool = False
    """If set to False, AxieStudio will not send progress notifications in the MCP server."""
    ...
    """If set to True, AxieStudio will only partially load components at startup and fully load them on demand.
    ...
    """If set to True, AxieStudio will create starter projects. If False, skips all starter project setup.
    ...
    """If set to True, AxieStudio will update starter projects."""
    ...
    def set_database_url(cls, value, info):
        ...
        if AXIESTUDIO_database_url := os.getenv("AXIESTUDIO_DATABASE_URL"):
            ...
            from axi.utils.version import get_version_info
            from axi.utils.version import is_pre_release as AXIESTUDIO_is_pre_release
