...
from axi.cli.serve_app import FlowMeta, create_multi_serve_app
...
def serve_command(
    ...
    *,
    ...
    """Serve AXI flows as a web API.

    Supports single files, inline JSON, and stdin input.

    Examples:
        # Serve from file
        axi serve my_flow.json

        # Serve inline JSON
        axi serve --flow-json '{"nodes": [...], "edges": [...]}'

        # Serve from stdin
        cat my_flow.json | axi serve --stdin
        echo '{"nodes": [...]}' | axi serve --stdin
    """
    ...
    try:
        api_key = get_api_key()
        verbose_print("✓ AXIESTUDIO_API_KEY is configured")
    except ValueError as e:
        typer.echo(f"✗ {e}", err=True)
        typer.echo("Set the AXIESTUDIO_API_KEY environment variable before serving.", err=True)
    ...
    os.environ["AXIESTUDIO_PRETTY_LOGS"] = "false"
    verbose_print(f"Configuring logging with level: {log_level}")
    from axi.log.logger import configure
    ...
    try:
        ...
        if resolved_path.suffix == ".json":
            ...
            from axi.cli.script_loader import load_graph_from_script
        ...
        try:
            ...
            if check_variables:
                from axi.cli.validation import validate_global_variables_for_env
        ...
        console.print(
            Panel.fit(
                ...
                title="[bold blue]axi server[/bold blue]",
                border_style="blue",
            )
        )
        console.print()
