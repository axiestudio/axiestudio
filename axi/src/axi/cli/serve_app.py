"""FastAPI application factory for serving **multiple** AXI graphs at once.
...
:func:`AXI.cli.commands.verify_api_key`.
"""
...
from axi.cli.common import execute_graph_with_capture, extract_result_data, get_api_key
...
# Security - use the same pattern as AxieStudio main API
...
def _analyze_graph_structure(graph: Graph) -> dict[str, Any]:
    ...
    Args:
        graph: The AXI graph to analyze
...
def _generate_dynamic_run_description(graph: Graph) -> str:
    ...
    description_parts = [
        f"Execute the deployed AXI graph with {analysis['node_count']} components.",
        "",
        "**Graph Analysis**:",
...
class RunRequest(BaseModel):
    """Request model for executing a AXI flow."""

    input_value: str = Field(..., description="Input value passed to the flow")


class StreamRequest(BaseModel):
    """Request model for streaming execution of a AXI flow."""
...
async def run_flow_generator_for_serve(
    ...
    try:
        ...
        # to integrate with the full AXI streaming pipeline from endpoints.py
...
def create_multi_serve_app(
    *,
    ...
    """Create a FastAPI app exposing multiple AXI flows.
    ...
    app = FastAPI(
        title=f"AXI Multi-Flow Server ({len(graphs)})",
        description=(
            "This server hosts multiple AXI graphs under the `/flows/{id}` prefix. "
    ...
    def create_flow_router(flow_id: str, graph: Graph, meta: FlowMeta) -> APIRouter:
        ...
        async def stream_flow(
            ...
            try:
                ...
                from axi.events.event_manager import create_stream_tokens_event_manager
