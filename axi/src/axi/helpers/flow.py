"""Flow helper functions for AXI package."""
...
if TYPE_CHECKING:
    ...
    from axi.graph.schema import RunOutputs
...
async def list_flows(*, user_id: str | None = None) -> list[Data]:
    ...
    In AXI, this is a stub that returns an empty list since we don't have
    ...
    # In AXI, we don't have a database backend by default
    # This is a stub implementation
    logger.warning("list_flows called but AXI doesn't have database backend by default")
    return []
...
async def load_flow(
    ...
    In AXI, this is a stub that raises an error since we don't have
    a database backend by default.
    ...
    # In AXI, we don't have a database backend by default
    # This is a stub implementation
    msg = f"load_flow not implemented in AXI - cannot load flow {flow_id or flow_name}"
...
async def run_flow(
    ...
    if graph is None:
        # In AXI, we can't load flows from database
        msg = "run_flow requires a graph parameter in AXI"
        raise ValueError(msg)
    ...
    # In AXI, we don't have settings service, so use False as default
