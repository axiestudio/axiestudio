...
console.log('🔥 FIXING ALL 458 MISSING NAMED EXPORTS...\n');
...
function addNamedExport(content, filePath) {
  // Look for lines like "export default ComponentName;"
  ...
  matches.forEach(match => {
    ...
    if (!newContent.includes(`export { ${componentName} }`)) {
      // Add named export at the very end of the file
...
if (totalFixed > 0) {
  console.log('\n🚀 ALL 458 EXPORT ISSUES RESOLVED! Ready to build!');
  console.log('🎯 Next step: Run "npm run build" to verify everything works!');
} else {
