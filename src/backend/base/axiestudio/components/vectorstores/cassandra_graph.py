...
class CassandraGraphVectorStoreComponent(LCVectorStoreComponent):
    display_name = "Cassandra Graf"
    description = "Cassandra Graf Vektorlager"
    name = "CassandraGraph"
    icon = "Cassandra"
    ...
    inputs = [
        MessageTextInput(
            name="database_ref",
            display_name="Kontaktpunkter / Astra Databas-ID",
            info="Kontaktpunkter för databasen (eller Astra DB databas-ID)",
            required=True,
        ),
        MessageTextInput(
            name="username", display_name="Användarnamn", info="Användarnamn för databasen (lämna tomt för Astra DB)."
        ),
        SecretStrInput(
            name="token",
            display_name="Lösenord / Astra DB Token",
            info="Användarlösenord för databasen (eller Astra DB token).",
            required=True,
        ),
        MessageTextInput(
            name="keyspace",
            display_name="Nyckelutrymme",
            info="Tabell Nyckelutrymme (eller Astra DB namnrymd).",
            required=True,
        ),
        MessageTextInput(
            name="table_name",
            display_name="Tabellnamn",
            info="Namnet på tabellen (eller Astra DB samling) där vektorer kommer att lagras.",
            required=True,
        ),
        DropdownInput(
            name="setup_mode",
            display_name="Installationsläge",
            info="Konfigurationsläge för att ställa in Cassandra-tabellen, med alternativ som 'Sync' eller 'Off'.",
            options=["Sync", "Off"],
            value="Sync",
            advanced=True,
        ),
        DictInput(
            name="cluster_kwargs",
            display_name="Klusterargument",
            info="Valfri ordbok med ytterligare nyckelordsargument för Cassandra-klustret.",
            advanced=True,
            list=True,
        ),
