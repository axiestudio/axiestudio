...
class CreateDataComponent(Component):
    ...
    def update_build_config(self, build_config: dotdict, field_value: Any, field_name: str | None = None):
        if field_name == "number_of_fields":
            ...
            if field_value_int > self.MAX_FIELDS:
                ...
                msg = (
                    f"Antal fält kan inte överstiga {self.MAX_FIELDS}. "
                    "Vänligen justera antalet fält för att vara inom den tillåtna gränsen."
                )
            ...
            for i in range(1, field_value_int + 1):
                ...
                if key in existing_fields:
                    ...
                    field = DictInput(
                        display_name=f"Fält {i}",
                        name=key,
                        info=f"Nyckel för fält {i}.",
    ...
    def get_data(self):
        """Funktion för att hämta Data från attributen."""
        data = {}
    ...
    def validate_text_key(self) -> None:
        """Denna funktion validerar att textnyckeln är en av nycklarna i Data."""
        data_keys = self.get_data().keys()
        if self.text_key not in data_keys and self.text_key != "":
            formatted_data_keys = ", ".join(data_keys)
            msg = f"Textnyckel: '{self.text_key}' hittades inte i Data-nycklarna: '{formatted_data_keys}'"
            raise ValueError(msg)
