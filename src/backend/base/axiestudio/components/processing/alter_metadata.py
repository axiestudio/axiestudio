...
class AlterMetadataComponent(Component):
    ...
    inputs = [
        HandleInput(
            ...
            info="Objekt som metadata ska läggas till",
            required=False,
        ...
        StrInput(
            name="text_in",
            display_name="Användartext",
            info="Textinmatning; värdet kommer att vara i 'text'-attributet för Data-objektet. Tomma textposter ignoreras.",
            required=False,
        ),
        NestedDictInput(
            name="metadata",
            display_name="Metadata",
            info="Metadata att lägga till för varje objekt",
            input_types=["Data"],
            required=True,
        ),
        MessageTextInput(
            name="remove_fields",
            display_name="Fält att ta bort",
            info="Metadatafält att ta bort",
            required=False,
            is_list=True,
        ),
    ]
    ...
    outputs = [
        Output(
            name="data",
            display_name="Data",
            info="Lista med inmatningsobjekt var och en med tillagd metadata",
            method="process_output",
        ),
        Output(
            display_name="DataFrame",
            name="dataframe",
            info="Data-objekt som en DataFrame, med metadata som kolumner",
            method="as_dataframe",
        ),
    ]
    ...
    def _as_clean_dict(self, obj):
        ...
        if isinstance(obj, dict):
            ...
            msg = f"Förväntade ett Data-objekt eller en ordbok men fick {type(obj)}."
            raise TypeError(msg)
