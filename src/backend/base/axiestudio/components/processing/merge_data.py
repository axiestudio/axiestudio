...
class MergeDataComponent(Component):
    display_name = "Kombinera data"
    description = "Kombinerar data med olika operationer"
    icon = "merge"
    MIN_INPUTS_REQUIRED = 2
    legacy = True

    inputs = [
        DataInput(name="data_inputs", display_name="Datainmatningar", info="Data att kombinera", is_list=True, required=True),
        DropdownInput(
            name="operation",
            display_name="Operationstyp",
    ...
    def combine_data(self) -> DataFrame:
        ...
        try:
            ...
            logger.error(f"Fel under operation {operation}: {e!s}")
            raise
        else:
