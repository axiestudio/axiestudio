...
class DoclingRemoteComponent(BaseFileComponent):
    ...
    inputs = [
        *BaseFileComponent._base_inputs,
        StrInput(
            name="api_url",
            display_name="Serveradress",
            info="URL för Docling Serve-instansen.",
            required=True,
        ),
        IntInput(
            name="max_concurrency",
            display_name="Samtidighet",
            info="Maximalt antal samtidiga förfrågningar till servern.",
            advanced=True,
            value=2,
        ),
        FloatInput(
            name="max_poll_timeout",
            display_name="Maximal väntetid",
            info="Maximal väntetid för att dokumentkonverteringen ska slutföras.",
            advanced=True,
            value=3600,
        ),
        NestedDictInput(
            name="api_headers",
            display_name="HTTP-rubriker",
            advanced=True,
            required=False,
            info=("Valfri ordbok med ytterligare rubriker som krävs för att ansluta till Docling Serve."),
        ),
        NestedDictInput(
