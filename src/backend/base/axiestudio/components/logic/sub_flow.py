...
class SubFlowComponent(Component):
    ...
    async def update_build_config(self, build_config: dotdict, field_value: Any, field_name: str | None = None):
        ...
        if field_value is not None and field_name == "flow_name":
            try:
                ...
                if not flow_data:
                    msg = f"Flöde {field_value} hittades inte."
                    logger.error(msg)
                else:
                    try:
                        ...
                        logger.exception(f"Fel vid byggande av graf för flöde {field_value}")
    ...
    inputs = [
        DropdownInput(
            name="flow_name",
            display_name="Flödesnamn",
            info="Namnet på flödet att köra.",
            options=[],
            refresh_button=True,
    ...
    outputs = [Output(name="flow_outputs", display_name="Flödesutmatningar", method="generate_results")]
