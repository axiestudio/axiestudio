...
class MCPToolsComponent(ComponentWithCache):
    ...
    async def _validate_schema_inputs(self, tool_obj) -> list[InputTypes]:
        """Validera och bearbeta schemainmatningar för ett verktyg."""
        try:
            if not tool_obj or not hasattr(tool_obj, "args_schema"):
                msg = "Ogiltigt verktygsobjekt eller saknat inmatningsschema"
                raise ValueError(msg)

            flat_schema = flatten_schema(tool_obj.args_schema.schema())
            input_schema = create_input_schema_from_json_schema(flat_schema)
            if not input_schema:
                msg = f"Tomt inmatningsschema för verktyg '{tool_obj.name}'"
                raise ValueError(msg)

            schema_inputs = schema_to_axiestudio_inputs(input_schema)
            if not schema_inputs:
                msg = f"Inga inmatningsparametrar definierade för verktyg '{tool_obj.name}'"
                logger.warning(msg)
                return []

        except Exception as e:
            msg = f"Fel vid validering av schemainmatningar: {e!s}"
