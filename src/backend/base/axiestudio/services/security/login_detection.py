"""
Login Detection and Security Notification Service (Swedish App)
...
class LoginDetectionService:
    ...
    def _parse_user_agent(self, user_agent: str) -> Dict[str, str]:
        ...
        try:
            ...
            if "mobile" in user_agent_lower:
                device_info["device"] = "Mobil"
            elif "tablet" in user_agent_lower:
                device_info["device"] = "Surfplatta"
            else:
                device_info["device"] = "Dator"
            
            return device_info
            
        except Exception as e:
            logger.error(f"Error parsing user agent: {e}")
            return {"browser": "Okänd", "os": "Okänt", "device": "Okänd"}
    ...
    async def _get_location_from_ip(self, client_ip: str) -> Dict[str, str]:
        ...
        try:
            ...
            return {"country": "Okänt", "city": "Okänt", "region": "Okänt"}
    ...
    async def _send_login_notification(
        ...
        try:
            ...
            if location_info['city'] == "Okänt":
                location = location_info['country']
            
            device = f"{device_info['browser']} på {device_info['os']} ({device_info['device']})"
