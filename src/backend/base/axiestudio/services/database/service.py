...
class DatabaseService(Service):
    ...
    def _fix_webhook_events_schema(self) -> None:
        ...
        ADDRESSES EXACT ERROR FROM LOG:
        - modify_nullable: webhook_events.created_at (False -> True)
        - remove_index: ix_webhook_events_created_at, ix_webhook_events_status
        - remove_constraint: UniqueConstraint on stripe_event_id (NullType)
        - remove_index: ix_webhook_events_stripe_event_id (VARCHAR)
        - add_index: ix_webhook_events_stripe_event_id (AutoString, unique=True)
        """
        from sqlalchemy import text, inspect
