...
class DatabaseService(Service):
    ...
    def _fix_webhook_events_schema_sync(self) -> None:
        """
        Synkron version av webhook_events schema fix.
        Detta hanterar både sync och async engines korrekt.
        """
        from sqlalchemy import text, inspect
        ...
        # Hantera både sync och async engines
        if hasattr(self.engine, 'sync_engine'):
            # AsyncEngine - använd sync_engine för sync operationer
            engine = self.engine.sync_engine
        else:
            # Redan en sync engine
            engine = self.engine
