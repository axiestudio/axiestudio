...
class DatabaseService(Service):
    ...
    def _add_enhanced_security_columns(connection) -> None:
        ...
        try:
            ...
            if added_columns:
                ...
                logger.info(f"✅ Successfully added {len(added_columns)} enhanced security columns: {added_columns}")
            else:
                logger.debug("All enhanced security columns already exist")

        except Exception as e:
            logger.warning(f"Enhanced security columns setup failed: {e}")
    ...
    def _create_db_and_tables(connection) -> None:
        ...
        if table_names and all(table in table_names for table in current_tables):
            ...
            logger.info(f"📊 Found {len(current_tables)} required tables: {', '.join(current_tables)}")
            # Still need to add enhanced security columns if they don't exist
            try:
                logger.info("🔒 Checking enhanced security columns...")
                DatabaseService._add_enhanced_security_columns(connection)
                logger.info("✅ Enhanced security columns verified")
            except Exception as exc:
                logger.warning(f"⚠️ Enhanced security columns setup failed: {exc}")
                # Don't fail the entire startup for this
            return
        ...
        total_tables = len(SQLModel.metadata.sorted_tables)
        logger.info(f"📋 Found {total_tables} table definitions to process")

        for i, table in enumerate(SQLModel.metadata.sorted_tables, 1):
            table_name = table.name
            logger.info(f"🔧 [{i}/{total_tables}] Processing table: {table_name}")
            ...
            try:
                ...
                if table_name not in table_names:
                    ...
                    logger.debug(f"⏭️ Table {table_name} already exists")
            except OperationalError as oe:
                skipped_tables.append(table_name)
                logger.debug(f"⏭️ Table {table_name} already exists, skipping. Exception: {oe}")
            except Exception as exc:
        ...
        # Report results
        if created_tables:
            logger.info(f"🎉 Successfully created {len(created_tables)} new tables: {', '.join(created_tables)}")
        if skipped_tables:
            logger.info(f"⏭️ Skipped {len(skipped_tables)} existing tables")
        if failed_tables:
            logger.error(f"❌ Failed to create {len(failed_tables)} tables: {', '.join(failed_tables)}")
        ...
        if missing_tables:
            logger.error("💥 Database table creation verification failed!")
            logger.error(f"📋 Missing required tables: {', '.join(missing_tables)}")
            logger.error("🔧 Please check your database settings and permissions.")
            msg = f"Failed to create required tables: {', '.join(missing_tables)}"
        ...
        try:
            logger.info("🔒 Adding enhanced security columns if missing...")
            DatabaseService._add_enhanced_security_columns(connection)
            logger.info("✅ Enhanced security columns setup completed")
        except Exception as exc:
            logger.warning(f"⚠️ Enhanced security columns setup failed: {exc}")
            # Don't fail the entire startup for this
