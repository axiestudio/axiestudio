...
class DatabaseService(Service):
    ...
    def _fix_webhook_events_schema(self) -> None:
        ...
        This method handles the specific issue where the database schema doesn't match the SQLModel definition.
        """
        from sqlalchemy import text, inspect

        logger.info("🔧 Attempting to fix webhook_events schema mismatch...")
        ...
        with self.engine.connect() as connection:
            ...
            try:
                ...
                logger.info("🔍 Found existing webhook_events table, checking schema...")

                # Drop problematic indexes that cause conflicts
                problematic_indexes = [
                ...
                for index_name in problematic_indexes:
                    try:
                        connection.execute(text(f"DROP INDEX IF EXISTS {index_name}"))
                        logger.debug(f"✅ Dropped problematic index: {index_name}")
                    except Exception as e:
                        logger.debug(f"⚠️ Could not drop index {index_name}: {e}")

                # Fix created_at column to be NOT NULL if it's nullable
                try:
                    ...
                    """))
                    logger.info("✅ Fixed created_at column to be NOT NULL")
                except Exception as e:
                    logger.warning(f"⚠️ Could not fix created_at column: {e}")

                # Recreate indexes with correct definitions
                index_queries = [
                ...
                for query in index_queries:
                    try:
                        ...
                        logger.debug(f"✅ Created index: {query.split()[5]}")
                    except Exception as e:
                        logger.debug(f"⚠️ Could not create index: {e}")

                trans.commit()
                logger.info("✅ webhook_events schema fix completed successfully")

            except Exception as e:
                trans.rollback()
                logger.error(f"❌ Failed to fix webhook_events schema: {e}")
                raise
