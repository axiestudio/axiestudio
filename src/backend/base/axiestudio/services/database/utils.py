...
async def initialize_database(*, fix_migration: bool = False) -> None:
    ...
    try:
        ...
        # Handle webhook_events specific migration conflicts
        if "webhook_events" in str(exc) or "AutogenerateDiffsDetected" in str(exc):
            logger.warning("🚨 WEBHOOK_EVENTS MIGRATION CONFLICT DETECTED")
            logger.warning("💡 Applying Langflow-style fix: dropping alembic_version table")
            async with session_getter(database_service) as session:
                await session.exec(text("DROP TABLE IF EXISTS alembic_version"))
                await session.commit()
