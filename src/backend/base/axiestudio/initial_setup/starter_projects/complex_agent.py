...
def complex_agent_graph():
    ...
    role_prompt = PromptComponent(_display_name="Rollprompt")
    role_prompt.set(
...
"""
    ...
    goal_prompt = PromptComponent(_display_name="Målprompt")
    goal_prompt.set(
...
""",
    ...
    backstory_prompt = PromptComponent(_display_name="Bakgrundsprompt")
    backstory_prompt.set(
        template="""Definiera en bakgrund för denna roll och mål, givet användarens fråga.
Användarens fråga: {query}

Roll: {role}
Mål: {goal}

Bakgrunden bör vara specifik och väl anpassad till resten av informationen.
Bakgrund:""",
        query=chat_input.message_response,
        role=role_prompt.build_prompt,
...
Svara användaren med så mycket information som möjligt om ämnet. Ta bort om det behövs.
